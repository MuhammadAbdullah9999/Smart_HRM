  ...
  const handleSubmit = async () => {
    ...
    employeeAttendance.forEach((employee) => {
      if (validator.isEmpty(employee.checkIn)) {
        validationErrors[`${employee.id}_checkIn`] =
          "Check-in field is required.";
      }
      if (validator.isEmpty(employee.checkOut)) {
        validationErrors[`${employee.id}_checkOut`] =
          "Check-out field is required.";
      }
    });
    ...
    const allData = employeeAttendance.map((employee) => {
      ...
      const month = selectedDateObject.toLocaleString("en-US", {
        month: "long",
      });
                ...
                <tr key={employee.id}>
                  ...
                  <td className="border border-gray-300 p-2 text-center">
                    <CheckCircleOutlineIcon
                      style={{
                        color: employee.status === "present" ? "green" : "gray",
                      }}
                      onClick={() =>
                        setEmployeeAttendance((prev) =>
                          prev.map((emp) =>
                  ...
                  <td className="border border-gray-300 p-2 text-center">
                    <CancelIcon
                      style={{
                        color: employee.status === "absent" ? "red" : "gray",
                      }}
                      onClick={() =>
                        setEmployeeAttendance((prev) =>
                        prev.map((emp) =>
